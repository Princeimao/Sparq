...
export function sanitizeInputForLlm(text: string): string {
  ...
  let sanitized = text;
  sanitized = sanitized.replace(/(\+?\d{1,4}[\s-]?)?(\d{10,15})/g, "[PHONE_MASKED]");
  sanitized = sanitized.replace(/\b[a-zA-Z0-9_-]{15,}\b/g, "[ID_MASKED]");
  sanitized = sanitized.replace(/(eaao|ghs|token|key|secret)[a-zA-Z0-9_]+/gi, "[TOKEN_MASKED]");
...
export const LlmTaskAnalysisSchema = z.object({
  intent: z
    ...
    .describe("The classified intent of the user's message. ORDER is for ordering a product. PROVIDE_ADDRESS is for sharing location/address details. CONFIRM/DECLINE are for yes/no actions. OTHER is for general queries."),
  productName: z
    .string()
    .optional()
    .describe("The product name extracted from the message if the user is attempting to place an order (e.g., 'chocolate oats')."),
  address: z
    .string()
    .optional()
    .describe("The delivery/shipping address extracted from the message if the user is sharing an address."),
});

export const ProductSearchSchema = z.object({
  intent: z.enum(["ORDER_PRODUCT", "OTHER", "ERROR"]).describe("The classified intent of the user's message. ORDER_PRODUCT is for ordering a product. OTHER is for general queries."),
  productQuery: z.string().nullable().describe("The product name extracted from the message if the user is attempting to place an order (e.g., 'chocolate oats')."),
  quantity: z.number().nullable().describe("The quantity of the product extracted from the message if the user is attempting to place an order (e.g., '2')."),
  headerMessage: z.string().describe("The header message for the interactive buttons."),
  footerMessage: z.string().describe("The footer message for the interactive buttons."),
...
export class LlmService {
  ...
  constructor() {
    ...
    if (!apiKey) {
      console.warn("WARNING: GEMINI_API_KEY is not set. Langchain Gemini LLM calls may fail.");
    }
  ...
  async analyzeMessage(rawMessageText: string): Promise<LlmTaskAnalysis> {
    ...
    const structuredModel = this.model.withStructuredOutput(LlmTaskAnalysisSchema);
